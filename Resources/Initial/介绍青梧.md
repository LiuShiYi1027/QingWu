...
1. 青梧支持 macOS 11.5 及以上版本，首次打开建议先在设置中确认默认存储目录
2. 在 iCloud、坚果云桌面同步目录或其他位置创建 `QingWu` 文件夹,打开设置修改默认存储地址
...
- 更多 Markdown 示例如需对照，可从侧边栏打开 **青梧 Markdown 语法指南**。
...
- 更完整的折叠块、图表、思维导图写法，可在 **青梧 Markdown 语法指南** 中查阅。

## 青梧 PPT

1. 新用户默认会生成模板，老用户升级后可手动创建「青梧 PPT」文档进行体验
2. 执行 `command + option + p` 启动 PPT 预览,或右键文档选择「青梧 PPT」
3. 只有包含 `---` 分隔符的文档可启用 PPT 模式,演示中按「回车」预览大纲,「ESC」退出
4. 可使用 HTML 自定义效果,复杂用法参考 [Reveal](https://revealjs.com/markdown/) 文档

## 为什么做青梧

之前尝试过众多笔记应用(为知笔记、印象笔记、Ulysses、Quiver、MWeb、Bear、Typora),始终没找到一款趁手的 Markdown 工具,于是有了做青梧的想法。

本职工作是前端开发,会一点 iOS,爱折腾,借青梧探索 Swift 和 macOS 开发,顺便做一个自己喜欢的工具。

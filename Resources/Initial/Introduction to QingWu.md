...
1. QingWu supports macOS 11.5 and later. On first launch, review your default storage location in Settings.
2. Create a `QingWu` folder in iCloud, a desktop cloud-drive folder, or your preferred location, then set it as default storage in Settings
...
- `command + option + m` - Show/hide QingWu globally
...
- For ready-made Markdown patterns, open **QingWu Markdown Syntax Guide** from the sidebar templates.
...
- Detailed syntax samples, including collapsible sections and diagram templates, live in **QingWu Markdown Syntax Guide**.

## QingWu PPT

1. New users get templates by default. Existing users can create a "QingWu PPT" note manually to try it out
2. Press `command + option + p` to launch PPT preview, or right-click a document and select "QingWu PPT"
...
After trying many note apps (WizNote, Evernote, Ulysses, Quiver, MWeb, Bear, Typora), I couldn't find a Markdown tool that felt right, so I built QingWu.

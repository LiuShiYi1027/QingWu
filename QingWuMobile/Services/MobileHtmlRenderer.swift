...
enum MobileHtmlRenderer {
    ...
    /// Rewrite local image references to the custom `qingwu-asset` scheme.
    ...
    /// `qingwu-wiki` scheme so the reader can navigate between notes.
    ...
    /// Scripts load through the `qingwu-bundle` scheme handler, so the
    ...
    private static func readerScripts(for body: String) -> String {
        ...
        if hasMermaid {
            ...
            out += """
                <script src="qingwu-bundle:///mermaid.min.js"></script>
                <script>
                ...
                var qingwuDark = window.matchMedia("(prefers-color-scheme: dark)").matches;
                mermaid.initialize({ startOnLoad: true, theme: qingwuDark ? "dark" : "neutral" });
                </script>
                """
        }
        if hasCodeBlock {
            out += """
                <script src="qingwu-bundle:///highlight.min.js"></script>
                <script>

#!/usr/bin/env ruby
# Wires standalone helper files into the QingWu target.
#
...
# QingWu target's source build phase.
#
# Usage:
#   gem install xcodeproj   # if not already installed
#   ruby scripts/wire_helper_files.rb
#
# Commit the resulting QingWu.xcodeproj/project.pbxproj diff afterwards.
require 'xcodeproj'
...
target = project.targets.find { |t| t.name == 'QingWu' }
abort 'QingWu target not found' unless target
...
FILES_TO_WIRE.each do |rel_path, group_name|
  ...
  # Idempotency: skip if any existing build file in QingWu's source phase
  ...
  if already_in_phase
    puts "skip: #{rel_path} already in QingWu target"
    skipped += 1
    next
  end

  # Locate the group: source files live under main_group -> QingWu -> <group_name>.
  parent = project.main_group.children.find { |c|
    c.is_a?(Xcodeproj::Project::Object::PBXGroup) && c.display_name == 'QingWu'
  }
  abort "QingWu source group not found in main_group" unless parent
  group = parent.children.find { |c|
    c.is_a?(Xcodeproj::Project::Object::PBXGroup) && c.display_name == group_name
  }
  abort "group not found under QingWu: #{group_name}" unless group
  ...
  puts "added: #{rel_path} (to QingWu target via group '#{group_name}')"
  added += 1
end
...
puts 'Next: commit QingWu.xcodeproj/project.pbxproj, then xcodebuild build to verify.'
